...
public class Student_Apply_ForCSP_UG_Degree_With_Subject extends TestBase {
	...
	private static By addDegree = By.xpath("//button[contains(text(),'Add degree')][1]");
	...
	public static void student_Apply_ForCSP_UG_Degree_With_Subject() throws Exception {
		...
		subjectEnroll("Academic Writing", "820");		
		click(addDegree);
		...
		//searchOUAID_PeopleSoft("1347071");
	...
	public static void subjectEnroll(String Subject1, String feez) throws Exception {
		...
		sendkeys(searchSubject_txtBox, Subject1);
		...
		Thread.sleep(5000);
		Screenshot.infoScreenshot(driver, Subject1 + " selected page");
		...
		Assert.assertTrue(fee.contains(feez));
	...
	public static void residencystatus(String TaxFilenumber) throws InterruptedException, AWTException, Exception {
		...
		System.out.println("noOfSub:-"+noOfSub);
		for (int i = 1; i <= noOfSub.size(); i++) {
			String SubjectName = driver.findElement(By.xpath("//div[@class='cart-summary-subjects cart-selected-subjects']/div[" + i + "]/p[3]")).getText();
			String SubjectPrice = driver.findElement(By.xpath("//div[@class='cart-summary-subjects cart-selected-subjects']/div[" + i + "]/p[1]")).getText();
		...
		Assert.assertTrue(totalCost.contains(Repository.getProperty("TotalCost2")));
		...
		click(fileUpload);
		Thread.sleep(3000);
		uploadFileUsingRobot("E:/surya automation/TestProject/SikuliScreenshots/OUA.png");
		test.log(LogStatus.INFO, "Work and life experience <b style=\"color:green;\"> 'document' </b> provided");
		log.info("Work and life experience 'document' provided");
		Thread.sleep(3000);
		...
		Thread.sleep(3000);
		Screenshot.capture(driver, " Undergraduate degree application ");
